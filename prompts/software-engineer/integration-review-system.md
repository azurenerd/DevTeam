...
6. **Data file wiring** — any data files the app reads at runtime (e.g., JSON config, sample data) exist, are copied to output (`<Content CopyToOutputDirectory="PreserveNewest" />` in csproj if needed), and are readable from the path the service expects. If the app expects a runtime data file, both a sample/template AND the actual file must be committed.
