...
public partial class PullRequestWorkflow
{
        ...
        CancellationToken ct = default)
    {
        ...
        // 2. Commit a placeholder so the branch has a diff from main (required for PR creation)
        _logger.LogInformation("Creating placeholder on branch {Branch} for {Path}", branchName, documentPath);
        await _github.CreateOrUpdateFileAsync(
            documentPath,
            $"# {prTitle}\n\n_Work in progress — being generated by {agentName}..._\n",
            $"[WIP] Start {documentPath}",
            branchName, ct);
        ...
        CancellationToken ct = default)
    {
        ...
        // 2. Update PR labels and description to show completion
        await _github.UpdatePullRequestAsync(pr.Number,
            labels: [Labels.ReadyForReview, Labels.Approved], ct: ct);

        // 3. Auto-merge (no review needed for initial docs)
        try
        {
